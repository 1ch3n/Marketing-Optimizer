# -*- coding: utf-8 -*-
"""auto_report_read_file.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1l4Q3YjRJ9MzIu0NAai5J2qVgUpqwxMIX
"""
...
def build_model_ols_rf(data, KPI, budget):
 ...
 # OLS Regression
    ...
    if coeffs['ShoppingReset_spend'] > 0 and coeffs['ResetSearch_branding_spend'] > 0 and coeffs['ResetAfterDrinkSearch_branding_spend'] > 0:
        ...
        sns.barplot(x=[Shopping_reset_budget, Reset_search_budget, ResetAfterDrinkSearch_budget], y=['ShoppingReset_spend', 'ResetSearch_branding_spend', 'ResetAfterDrinkSearch_branding_spend'], palette="viridis")
        plt.title('Budget Allocation based on Logistic Regression Coefficients')
        plt.xlabel('Allocated Budget')
        plt.ylabel('Campaigns')
        plt.show()
    else:
        plt.figure(figsize=(12, 6))
        coeffs = pd.Series(ols_model.coef_[0], index=ols_model.feature_names_in_)
        coeffs_sorted = coeffs.sort_values(ascending=False)
        sns.barplot(x=coeffs_sorted.values, y=coeffs_sorted.index, palette="viridis")
        plt.title('Budget Allocation based on Logistic Regression Coefficients')
...
def build_model_lr_rf(data, KPI, budget):
        X = data[['ResetSearch_branding_spend','ResetAfterDrinkSearch_branding_spend','ShoppingReset_spend','quarter_1',
            'quarter_2', 'quarter_3','dutch_sports_event','dutch_hol_fest']]
        Y = data[KPI]
   # Logistic Regression
        ...
        if coeffs['ShoppingReset_spend'] > 0 and coeffs['ResetSearch_branding_spend'] > 0 and coeffs['ResetAfterDrinkSearch_branding_spend'] > 0:
            ...
            plt.figure(figsize=(12, 6))
            coeffs = pd.Series(logit_model.coef_[0], index=logit_model.feature_names_in_)
            coeffs_sorted = coeffs.sort_values(ascending=False)
            import seaborn as sns
            sns.barplot(x=coeffs_sorted.values, y=coeffs_sorted.index, palette="viridis")
            plt.title('Budget Allocation based on Logistic Regression Coefficients')
...
def marketing_optimizer(pathname, KPI, budget):
    data = data_reader(pathname)
    if KPI == 'engagedSessions_reset':
        ols_model, rf_rscv, _ = build_model_ols_rf(data, KPI, budget)
    elif KPI == 'conversions_reset':
        logit_model, rf_rscv, _ = build_model_lr_rf(data, KPI, budget)

mo = marketing_optimizer(pathname='dataset.csv', KPI='engagedSessions_reset', budget=1000)